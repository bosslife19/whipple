...
const ConFirmSelectedLuckyNumbers = () => {
  ...
  const { odds, gameLabel, GameName, stake, selected, totalOdds } =
    gameData || {};
      ...
      <ScrollView contentContainerStyle={ConfirmsSTy.container}>
            ...
            {Array.from({ length: parseInt(range) }, (_, i) => i + 1).map(
              (num) => (
                ...
                >
                  <Text
                    style={[
                      ConfirmsSTy.numberText,
                      selectedNumbers.includes(num) &&
                        ConfirmsSTy.selectedTexts,
                    ]}
                  >
                    {num}
                  </Text>
          ...
          {success === false ? (
            ...
            >
              {loading ? (
                ...
                <Text style={ConfirmsSTy.primaryBtnText}>
                  Submit Your Number
                </Text>
              )}
            </TouchableOpacity>
        ...
        <View style={ConfirmsSTy.cards}>
          ...
          <Text style={ConfirmsSTy.descriptions}>
            Pick the exact{" "}
            <Text style={ConfirmsSTy.highlight}>{selected} number</Text> between{" "}
            <Text style={ConfirmsSTy.highlight}> {gameLabel}</Text> the House
            selected to win. If your selection matches exactly,{" "}
            <Text style={ConfirmsSTy.success}>you win!</Text>
          </Text>
        </View>

        <View style={ConfirmsSTy.buttonRow}>
          <TouchableOpacity
            style={ConfirmsSTy.secondaryBtn}
            onPress={() => router.back()}
          >
